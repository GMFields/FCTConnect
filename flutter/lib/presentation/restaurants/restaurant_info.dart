...
class RestaurantInfoPage extends StatefulWidget {
  ...
  const RestaurantInfoPage(
      {Key? key, required this.restaurant, required this.authToken})
      : super(key: key);
...
class _RestaurantInfoPageState extends State<RestaurantInfoPage> {
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => ReviewListPage(
                  reviews: reviews, restaurantName: widget.restaurant.name),
            ),
          );
        },
        backgroundColor: kPrimaryColor,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final result =
                  await addDish(widget.authToken, dish, widget.restaurant.name);
  ...
  Widget _buildDishesList(
      String title, Future<List<Dish>> Function(String, String) fetchDishes) {
  ...
  void _deleteDish(Dish dish) async {
    final result =
        await deleteDish(widget.authToken, widget.restaurant.name, dish.dishID);
    if (result == "success") {
      _dishFormController.clear();
      Navigator.of(context).pop();
      setState(() {});
